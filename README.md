...
Note: Vercel treats every `.py` file placed inside `api/` as a serverless entrypoint and expects a top-level `handler` or `app`. Shared helper modules should be placed in a root `/lib` directory rather than directly inside `api/`.
