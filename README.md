...
│   ├── _topic_detector.py <- shared helper (underscore prefix = not routed)
│   └── _gcd_solver.py     <- shared helper (underscore prefix = not routed)
├── public/
...
Files/folders starting with `_` inside `api/` are NOT turned into routes by
Vercel — that's intentional, it's how the shared solver/detector logic gets
imported without becoming its own (broken) endpoint.
...
4. Vercel auto-detects `api/*.py` as Python serverless functions via
   `vercel.json` and serves `public/` as static files.
